...
public class PhaseTwoScene: SKScene, ARSessionDelegate, ARSKViewDelegate {
    ...
    var upAnchor:ARAnchor!
    var downAnchor:ARAnchor!
    var rightAnchor:ARAnchor!
    var leftAnchor:ARAnchor!
    ...
    var motionLevelNumber = 1.0
    
    var alreadyMadeMove = false
    
    var motionManager = CMMotionManager()
    var motionThreshhold:Double = 1.5
    ...
    public override func didMove(to view: SKView) {        
        ...
        //motionLevelNumber = Double(getCurrentLevel())
        ...
        countdown = SKAction.repeat(decrementTime, count: 61)
        
        moveEyeDirContinuously = SKAction.repeatForever(SKAction.sequence([SKAction.run{self.changeEyeDirectionRandomly()}, SKAction.wait(forDuration: 1.0/motionLevelNumber), SKAction.run {
            self.alreadyMadeMove = false
            }]))
        
        briefBlinkWait = SKAction.wait(forDuration: 0.7)
    ...
    func resetApplePositions(){
        upApple.position = CGPoint(x: 0.0, y: 480.0)
        downApple.position = CGPoint(x: 0.0, y: -480.0)
        rightApple.position = CGPoint(x: 480.0, y: 0.0)
        leftApple.position = CGPoint(x: -480.0, y: 0.0)
    }
    ...
    public override func update(_ currentTime: TimeInterval) {
        if let data = self.motionManager.accelerometerData?.acceleration{
            var xAccel = data.x + 1.0
            if (xAccel > motionThreshhold || xAccel < -1.0*motionThreshhold || data.y > motionThreshhold || data.y < -1.0*motionThreshhold){
                
                if (xAccel < -1.0*motionThreshhold){
                    if isUpApple{
                        print("yeet")
                ...
                if (data.y < -1.0*motionThreshhold){
                    if isLeftApple{
                        print("yeet")
                        if (!alreadyMadeMove){
